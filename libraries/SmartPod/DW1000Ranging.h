...
public:
	...
	static void    startAsManchor(char address[], const byte mode[], const bool randomShortAddress = true);
	static void    startAsTag(char address[], const byte mode[], const bool randomShortAddress = true);
	static void    startAsTanchor(char address[], const byte mode[], const bool randomShortAddress = true);
...
private:
	...
	//mode (direct or blocked)
	static bool			_directPath;						// True for direct path, false for block path

	//variables for calibration/setup phase
	static byte         _firstTanchorShortAddress[2];		// Short address of B (first tanchor)
	static byte         _secondTanchorShortAddress[2];		// Short address of C (second tanchor)
	static float		_distanceAB;						// Distance between A (main anchor) and B (first tanchor)
	static float		_distanceAC;						// Distance between A (main anchor) and C (second tanchor)
	static float		_distanceBC;						// Distance between B (first tanchor) and C (second tanchor)
